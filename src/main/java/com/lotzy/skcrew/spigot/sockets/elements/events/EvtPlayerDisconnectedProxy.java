...
public class EvtPlayerDisconnectedProxy extends SkriptEvent {
    ...
    static {
        Skript.registerEvent("PlayerDisconnectedProxy", EvtPlayerDisconnectedProxy.class, PlayerDisconnectedProxyEvent.class,
                "player (leave|disconnect[e])[(d|s)] [from] proxy [server]");


        EventValues.registerEventValue(PlayerDisconnectedProxyEvent.class, OfflinePlayer.class,
                event -> event.getPlayer(), 0);
    }
